...
def reset_tables():
    ...
    u.xdb("DROP TABLE IF EXISTS budget")
...
u.ips("ledger_parts.data : get_records, get_budgets")
...
u.ips("ledger_parts.report : make_report")
...
def main():
    ...
    u.wjb(records, "ledger_records")
    u.wjb(budgets, "ledger_budgets")
    u.wcb(records, "ledger_records")

    loaded_records = u.rjb("ledger_records")
    loaded_budgets = u.rjb("ledger_budgets")
    ...
    u.wjb(category_summary, "ledger_category_summary")
    u.wjb(daily_summary, "ledger_daily_summary")
    u.wjb(budget_summary, "ledger_budget_summary")
    u.wcb(budget_summary, "ledger_budget_summary")

    report = make_report(category_summary, daily_summary, budget_summary)
    u.wtb(report, "ledger_report")
...
u.xpb("examples.ledger_app.main")
